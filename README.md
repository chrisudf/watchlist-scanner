...
| B 正常 | 1.0-1.25 | 1.0-1.35 | 🟢 照出, deep ITM 正合剧本 |
| C 贵 | > 1.25, 或 IV > 1.25 × max(近1年, 近2年实际波动) 升上来 | 同左, 线是 1.35 | 🟡 改 spread/PMCC, 候选合约仍列出 |
...
- **指数口径 C 线 1.35**: 成分股涨跌不同步, 实际波动被分散压低, 期权却要为
  "一起跌"收钱, 指数 IV/实际波动的常态比个股高一截。note 写"指数口径 C 线 1.35 倍"。
...
阈值 1.0 / 1.25 (指数 1.35) 是判断值。流水账记下每张票的 `iv_band` / `iv_ratio`,
