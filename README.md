...
### 扫描器侧的两级 OI 与 BE% 门 (2026-09-22)

上面那三张旗标票暴露的是扫描器的问题, 不是复盘表的问题。已改:

| 门 | 阈值 | 性质 | 在哪生效 |
|---|---|---|---|
| `leap_hard_min_oi` | 50 | **硬拦** | 挑票**之前**剔候选, 软回落救不回来 |
| `leap_min_oi` | 500 | 软门 | `passes()`, 不过带旗标照出 |
| `leap_max_be_pct` | 12% | 软门 | `passes()`, 不过带旗标照出 |
| `leap_max_extrinsic_pct` | 40% | 软门 | `passes()`, 不过带旗标照出 |

**两级 OI 是两件事, 别混。** 500 是"流动性弱"的质量线 —— 挂 mid 磨一磨还是
能成交的, 值得提醒但不该替人做决定。50 以下是"这张根本没有市场": 2026-09-21
的 TSM 340C 是 OI=1 的真事, 却作为 🟢 动作项发了出去。**买不到的票不该出现在
动作项里**, 所以硬拦那一条在 `clean`/`pick` 之前就把候选剔掉; 整条链都在地板
之下就整票 skip 并说明最厚的一档有多厚。

**BE% 是软门**, 和筛选器口径一致 (moomoo「Buy LEAP Call」的 0~12%)。它进
`passes()` 的效果是**换一张更深的**: delta 带内越深的行权价外在价值越薄, 带内
若有过线的就优先选过线的; 全带都过不了才回落并挂旗标。
