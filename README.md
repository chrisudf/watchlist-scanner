...
| `LEAP被拦` `CSP被拦` | 想出票被其他过滤器拦 (年化不足/流动性等), 原因见 ⏸ 行/详情 |
| `IV过门·不用LEAP` | LEAP 真票的平值 IV 过 58% 绝对门 (IV 档位 D) — 改 spread / sell put / 正股。C 档照常出票, 见下文「LEAP 的 IV 档位」 |
...
| **CONFIRMED** | 右侧确认 | 有回调前提 (近15日收盘曾低于20日线, 或距60日高回撤 ≥8%) **且**三选二 ≥2 项: ① 不再新低 (近5日最低 > 前15日最低) ② 放量收复20日线 (收盘>20日线 + 近10日曾在线下 + 量比 ≥1.5) ③ 突破前20日高 | LEAP 票 (仅 NORMAL regime; 财报 ≤14 天会拦; IV 档位 C/D 改写成 spread 提示) |
...
- **CONFIRMED** (新确认, 正常 regime) → 出 LEAP 票: 450-1100 DTE 优先
  ...
  15-30 天出票带提示; **IV 档位 C/D** 时改建议 spread/PMCC (D 档不用
  LEAP), 见下文「LEAP 的 IV 档位」。
...
### 扫描器侧的门与到期选择 (2026-09-23)

上面那三张旗标票暴露的是扫描器的问题, 不是复盘表的问题。查下去发现**根因不在
门槛, 在到期选择**。

#### 先说被证伪的那版

第一版据 `OI=1` 设了"可成交地板"硬拦。**错的** —— 同一天的报告原文:

| | OI | 价差 |
|---|--:|--:|
| TSM 2029-01-19 340C | **1** | **2.0%** |
| AAPL 2028-01-21 260C | 2284 | 2.3% |
| COHR 2028-01-21 240C | 97 | **5.7%** |

2.0% 的双边报价说明做市商在真报价。**OI 量的是已累积持仓, 价差量的才是当下
能不能成交** —— 按 OI 拦会拦掉报价最紧的那张、放行报价最松的那张。

#### 真正的根因: 到期选在了刚上市的周期

`2029-01-19` 在全部报告里首次出现就是 2026-09-21 —— 那个 LEAP 周期刚上市,
整条链 OI 接近 0。而到期是按"最接近 730 DTE"选的:

```
从 2026-09-21 算: 2028-01-21 = 487 天   |487-730| = 243
                  2029-01-19 = 851 天   |851-730| = 121   ← 选了这个
```

**扫描器在新周期上市的当天就跳了过去**, 而成熟的 2028 链 (TSM OI 2816 /
价差 2.2%) 就在旁边。这不是"合约不好", 是"周期还没开张"。

`_pick_leap_expiry` 的规则: 从最近的那个到期起, 在 `leap_exp_dte_slack`
(250 天) 窗口内按距离顺序看, 只要有一条链厚 `leap_exp_depth_ratio` (10) 倍
就让给它, 并把让位理由写进票据 notes。**两条都成熟时厚度差不到一个量级,
730 的偏好照旧生效** —— 这条规则只在"一条几乎是空的"时才咬合。

#### 门: 全是软门

| 门 | 阈值 | 说明 |
|---|---|---|
| `leap_min_oi` | 500 | 代价在**平仓端**: 无二级市场深度, 早期平仓只能对做市商 |
| `leap_max_spread_pct` | 5% | **能不能成交看这条**, 不是看 OI |
| `leap_max_be_pct` | 12% | 到盈亏平衡的幅度 (moomoo 筛选器同款 0~12%) |
| `leap_max_extrinsic_pct` | 40% | 外在价值占权利金 |

不过滤就带旗标照出, 由人判断 —— 唯一的硬门是财报缓冲。`leap_thin_oi` (50)
只决定 note 的措辞 (「几乎无二级市场」vs「流动性弱」), 不决定去留。

BE% 进 `passes()` 的效果是**换一张更深的**: delta 带内越深外在价值越薄,
带内若有过线的就优先选过线的; 全带都过不了才回落并挂旗标。

复盘表的 `旗标` 列同步加了价差 (`LEAP_GATES`), 和 OI 并排摆着 —— 免得再把
OI 当成可成交性读。

> ITM call 有恒等式 **BE = 现价 + 外在价值**, 所以 BE% 和 `extrinsic_pct`
> 量的是同一件事, 只是分母不同 (前者除现价, 后者除权利金)。调其中一条前
> 先想清楚另一条会不会跟着动。

⚠️ **12% 是绝对幅度, 对长年份 LEAP 偏严。** 同样 16% 的 BE, 486 天那张折年
12%/年, 851 天那张只有 6.9%/年 —— 后者其实更便宜, 却被同一条线判超标。所以
旗标 note 里一并给出折年数 (报告正文本来就有的 `保险费率 ~X%/年`), 看到超标
先看折年再决定。想改成按折年过滤, 动 `insurance_pct_yr` 那一路。

### LEAP 的 IV 档位 (2026-09-25)

LEAP 真票发出时, 扫描器取该到期日的平值 IV (离现价最近的行权价, call/put
各反解再取均值), 除以**这只票自己**过去 10 年滚动实际波动的中位数, 得出比值。
滚动窗口长度 = 合约剩余期限 (上限 24 个月)。

| 档位 | 个股 | 指数 (kind="index") | 报告怎么写 |
|---|---|---|---|
| A 便宜 | 比值 < 1.0 | < 1.0 | 🟢 照出 deep ITM, note 提示也可改用平值凸性档 |
| B 正常 | 1.0-1.25 | 同左 | 🟢 照出, deep ITM 正合剧本 |
| C 贵 | > 1.25, 或 IV > 1.25 × max(近1年, 近2年实际波动) 升上来 | 同左 | 🟢 照出 deep ITM, 行尾标"IV C 档: 只做深度实值"; note: 不买平值/虚值、不等回落, 个股仓位减半 |
| D 绝对门 | 平值 IV ≥ 58% | 同左 | 🟡 不用 LEAP, 改 spread / sell put / 正股 |

为什么不再用自建 IVP > 60: 那是 30 天口径, 两个方向都会骗人 —— 2026-09 财报后
NVDA 的 IV30 掉了 ~10 点, Jan'28 只掉 ~2.5 点; IBM 的 IV30 因为财报在 30 天内
反而高过 LEAP。IV 绝对值也不跨标的比: 2026-09-22 NVDA / IBM 的 Jan'28 平值 IV
都是 ~39%, 比值却是 0.78 / 1.67。

C 档为什么照常出票 (2026-09-25 改, 之前是 🟡 改 spread/PMCC 或等回落): SPX/NDX
2007-2026 指数代理回测 (`research/leap_iv_index_backtest.py`, 结果见 todo.md #3) 里,
深度实值在 C 档为"IV 贵"多付的成本约 1%; 平值多亏 8-12% 的权利金; "等回落"中位
要等 5 个月, 期间指数已涨 6-8%, 八成以上等完指数更高。spread 按权利金收益率与深度
实值相近、胜率略高, 但封顶, 且模型没加 skew 高估了它 —— 留作可选。个股没有回测
过 (高 IV 可能是个股自己的风险, 不一定修复), 所以个股 C 档仓位减半。

为什么是比值不是分位 (第一版用的是分位, 详见 lesson.md 2026-09-25): 长窗口的
实际波动分布很窄, IV 只比中位数高 ~15% (今天多数大票都这样) 在分位上就是 81-99。
回放过去 8 张 LEAP 推荐, 分位法拦了 6 张, 其中 3 张就是这种; 同一只 TSM 换个
到期日, 分位从 81 跳到 94。比值不随窗口长度变。

报告里的标注 (详见 lesson.md 2026-09-24 / 25):

- **指数口径**: 阈值单列一组 (`leap_iv_ratio_bands_index`), 目前与个股同线 1.25。
  曾按 QQQ 一天的截面设成 1.35, 9/25 的 SPX/NDX 20 年指数代理回测不支持, 改回。
  note 写"指数口径 C 线 1.25 倍"。
- **升档线就是 C 线**: 相对近 1-2 年实际波动也过了 C 线才升档。第一版升档是 1.15,
  比 C 线还严, 近期波动 ≈ 长期的稳定标的实际上 1.15 倍就被判贵。
- **长期中位数可能过时**: C 档且 IV 低于近 1-2 年实际波动的较高者时 (GLD 型: 黄金
  波动近两年翻倍), note 提示可人工降一档, 但要写理由。
- **贴线**: 比值离分界 < 0.03, 或平值 IV 离 58% 绝对门 < 2 个点时, note 写明
  "读数噪声就能翻档" (9/25 的 SOFI 58.5%、ISRG 1.25)。只标注, 不改档位。
- **IV 先从 bid/ask mid 反解**, 反解不出 (多见于有股息的深度实值) 才用 Yahoo 的
  IV 列。Yahoo 列对深度实值系统性偏高约 9 个点, delta 也跟着偏低。票上的合约 IV
  后面标 `(mid反解)` 或 `(⚠Yahoo列)`; 盘口不可用时反解的是最近成交价, 标
  `(成交价反解)`, 平值 IV 的 note 写"(含成交价)"。流水账记 `iv_src`。

**这些阈值有多少依据** (详见 lesson.md 2026-09-25 "更正"一节): **没有回测**。
"IV 对比自身历史实际波动"这个思路有实证 (Goyal & Saretto 2009, 但研究的是 1 个月
期期权); 指数门槛更高的方向有依据 (相关性风险溢价, Driessen 等 2009), 高 0.10 是
拍的。1.25 **不是**"正常风险溢价" —— 长期限的波动消息基本不定价 (Dew-Becker 等
2017), 今天比值 1.1-1.2 主要是近两年波动高于 10 年常态, 所以 1.25 的含义是"容忍
当前波动水平比常态高多少"。58% 是从 LEAP 筛选器约束 (有效杠杆 ≥2 等) 推出来的。

阈值 1.0 / 1.25 是判断值; 1.25 这条线有 SPX/NDX 指数代理回测的支持 (中偏弱,
`research/leap_iv_index_backtest.py`, 结果见 todo.md #3), 个股没有验证过。流水账记下每张票的 `iv_band` / `iv_ratio`,
复盘表的旗标列会标出 `IVC档` / `IVD档`, 攒够样本再校准。阈值都在 `[settings]`:
`leap_iv_ratio_bands` / `leap_iv_ratio_bands_index` / `leap_iv_abs_gate` /
`leap_iv_edge_ratio` / `leap_iv_edge_abs` / `leap_iv_hist_years` / `leap_iv_min_years`。
与 stock-analysis 技能第 4 步 (`leap_iv_gauge.py`) 同口径。
...
- 财报日期来自 yfinance, 获取失败时票据会带"自查"提示 — 下单前照惯例
  核对 moomoo 财报日历。
- 自建 IVP 需要 60 个交易日积累, 和 moomoo IVP 一样是 30 天口径, 只管 CSP。
  LEAP 的 IV 档位每张真票多一次 10 年日线请求; 取不到时票据写明"无读数",
  **不回落到 30 天 IVP**。
