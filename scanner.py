...
SETTINGS_DEFAULTS = {
    ...
    # 指数的阈值单列一组 (只管 kind="index"; GLD/DRAM 这类 etf 仍走个股线)。
    # 曾按 QQQ 一天的截面把指数 C 线设成 1.35 (相关性风险溢价); 9/25 用 SPX/NDX
    # 2007-2026 的指数代理回测 (research/leap_iv_index_backtest.py) 检验: 两者都是
    # 比值过 ~1.25 后 IV 才系统性高估事后波动, 不支持更宽的指数线, 改回与个股
    # 同线。设置保留, 便于以后有证据时分开 (lesson.md 2026-09-25 "指数 C 线")。
    ...
    "leap_iv_ratio_bands_index": [1.0, 1.25],
...
def leap_iv_band(atm_iv: float, closes, dte: int, s: dict,
    ...
    if atm_iv is None or len(rets) < h + 60:
        return None
...
def action_block(results: list[dict]) -> list[str]:
...
def render_close(results, regime, ivdf, now_et) -> str:
    ...
    for r in ordered:
        ...
        lines.append(
            ...
            f"| {STATE_LABEL[r['state']]} | {action_label(r)} "
